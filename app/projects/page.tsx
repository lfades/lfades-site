      ...
      <Terminal title="luis@alvarez: ~/projects">
        <div className="p-4">
          <div className="mb-4 text-ctp-green">
            <span className="text-ctp-blue">luis@alvarez</span>:<span className="text-ctp-mauve">~/projects</span>$ ls
            -la
          </div>
          ...
          <div className="space-y-6">
            {projects.map((project, index) => (
              ...
              >
                <h2 className="text-xl font-bold text-ctp-mauve mb-2">{project.name}</h2>
                <p className="text-ctp-text mb-3">{project.description}</p>

                <div className="flex flex-wrap gap-2 mb-3">
                  {project.tech.map((tech, techIndex) => (
                    <span key={techIndex} className="bg-ctp-surface1 text-ctp-subtext0 px-2 py-1 rounded text-xs">
                      {tech}
                    </span>
                  ))}
                </div>

                <Link href={project.link} className="text-ctp-blue hover:text-ctp-lavender hover:underline text-sm">
          ...
          <div className="mt-4 text-ctp-green">
            <span className="text-ctp-blue">luis@alvarez</span>:<span className="text-ctp-mauve">~/projects</span>${" "}
